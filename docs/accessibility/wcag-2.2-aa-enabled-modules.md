...
**Extended:** 2026-08-08 (backlog D4) — the Awards module promotion (`config/features.ts` `awards: true`) adds its programs dashboard page to the conformance scope and to the axe smoke.
**Extended:** 2026-08-08 (backlog D5) — the Workspaces module promotion (`config/features.ts` `workspaces: true`) adds its dashboard directory page to the conformance scope and to the axe smoke. With this last promotion the queue from ADR-0008 / §13.4 is fully drained: all eleven modules are enabled and in scope.

## Scope

Per ADR-0008, the WCAG 2.2 AA surface is the **enabled** modules only; flag-off modules are excluded from conformance for 1.0 because feature flags shrink the surface that must conform. Eleven modules are enabled as of the D5 workspaces promotion (2026-08-08) — every module in the registry, so this record now covers the whole dashboard surface.
...
| awards     | enabled                   | `/dashboard/awards/programs`                                                                                                                                                                                |
| workspaces | enabled                   | `/dashboard/organization/workspaces`                                                                                                                                                                        |
| (public)   | —                         | `/events`, `/jobs`                                                                                                                                                                                          |
...
No modules remain flag-off: the D5 workspaces promotion moved the last excluded module into scope, so every module is axe-audited and part of this conformance claim. Finance, chapters, committees, learning, awards and workspaces were in the flag-off list until their C5/D1/D2/D3/D4/D5 promotions.
...
Final run for the D5 promotion extension (2026-08-08): **all 18 pages PASS — 0 critical/serious, 0 moderate/minor**, including the new workspaces directory page with no axe violations found (raw results: `/tmp/nuvia-a11y-smoke-2026-08-08T14-44-22-299Z-468743/`). This is the full-registry run: every enabled module's representative page plus the two public listings.
...
- **Contrast of future colors.** The gate catches contrast regressions only on the eighteen audited pages; new components using low-opacity text utilities elsewhere will only be caught if those pages join the audit list.
