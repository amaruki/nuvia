...
Per ADR-0008, the WCAG 2.2 AA surface is the **enabled** modules only; flag-off modules are excluded from conformance for 1.0 because feature flags shrink the surface that must conform. Ten modules are enabled as of the D4 awards promotion (2026-08-08).
...
Flag-off modules (`workspaces`) are **not** axe-audited and are not part of this conformance claim. Finance, chapters, committees, learning and awards were in this list until their C5/D1/D2/D3/D4 promotions moved them into scope.
...
4. Runs `AxeBuilder` (`@axe-core/playwright`, tags `wcag2a`, `wcag2aa`, `wcag21a`, `wcag21aa`, `wcag22aa`) against the seventeen pages above.
...
- **Contrast of future colors.** The gate catches contrast regressions only on the seventeen audited pages; new components using low-opacity text utilities elsewhere will only be caught if those pages join the audit list.
