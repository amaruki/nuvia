...
Use one rate limiter, backed by Redis (`ioredis`, already a dependency through `src/lib/session-cache.ts`). It uses a sliding-window counter keyed by `ip + route`. Apply it to every `/api/v1/auth/*` route (login, signup, forgot-password, reset-password, change-password) and to any route that `docs/security/controls.md` marks as sensitive.
