...
- The permission taxonomy in `src/types/role/index.ts` (14 roles, 12 permission modules) is a good fit for the actual authorization needs of an AMS (for example, treasurer, chapter_president, committee_chair). This taxonomy does not need a redesign. This taxonomy needs full integration only. The code defines custom roles today. The code never resolves custom roles today (`TODO.md` M1).
</content>
