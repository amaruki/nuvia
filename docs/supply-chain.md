...
- **`socket.io`/`socket.io-parser`/`ws`/`engine.io`/`minimatch`/`esbuild`** (high, high, high/moderate, high, high, moderate): all of these are transitive dependencies, reachable only through `react-email` (its bundled preview server and its own build tooling) and `shadcn` (the CLI, `ts-morph`). Neither `react-email` nor `shadcn` runs in the deployed application. The application sends email through `nodemailer`/`resend` (`src/lib/auth.ts`'s `EmailServiceType` picker), never through `react-email`'s development server. `shadcn` is a one-time component generator that a developer invokes manually, not a runtime dependency. **Not reachable as deployed.**
