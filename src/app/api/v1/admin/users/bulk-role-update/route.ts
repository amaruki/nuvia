...
export async function POST(request: NextRequest) {
  try {
    ...
    const bulkUpdateSchema = z.object({
      userIds: z.array(z.string()).min(1, 'At least one user ID is required').max(100, 'Cannot update more than 100 users at once'),
      role: z.string().min(1, 'Role is required'),
      reason: z.string().optional(),
      confirm: z.boolean().refine(val => val === true, 'Confirmation is required for bulk operations')
    });
    ...
    if (userIds.includes(currentUser.id)) {
      return AuthResponseFactory.error(AuthErrorType.BUSINESS_LOGIC, 'Cannot update your own role in bulk operations');
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userIdsParam || !newRole) {
      return AuthResponseFactory.error(AuthErrorType.VALIDATION, 'userIds and role parameters are required');
    }
    ...
    const cannotManage = users.filter(user =>
      !canManageRole(currentUser.role, user.role as any) ||
