        ...
        <div className="p-6 space-y-6 animate-fadeIn">
                        ...
                        {filteredCertificates.length > 0 ? (
                            filteredCertificates.map((cert) => (
                                <TableRow key={cert.id} className="hover:bg-muted/5">
                                    <TableCell className="font-mono text-xs font-medium text-primary">{cert.verificationCode}</TableCell>
                                    <TableCell>
                                    ...
                                    <TableCell className="max-w-[200px] truncate font-medium text-sm" title={cert.courseName}>{cert.courseName}</TableCell>
                                    ...
                                    <TableCell>
                                        ...
                                        >
                                            <span className={`w-1.5 h-1.5 rounded-full mr-2 ${cert.status === 'active' ? "bg-green-600" : "bg-destructive"}`} />
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                ...
                                                <DropdownMenuItem asChild>
                                                    <Link href={`/dashboard/learning/certifications/${cert.id}`} target="_blank" className="cursor-pointer">
                                                        <Eye className="mr-2 h-4 w-4" /> View Details
                                                    </Link>
                                                </DropdownMenuItem>
                                                <DropdownMenuSeparator />
                                                <DropdownMenuItem className="text-destructive focus:text-destructive cursor-pointer" onClick={() => handleRevoke(cert.id)}>
            ...
            <div className="flex items-center justify-end space-x-2 text-sm text-muted-foreground">
                <div className="flex-1 text-xs">
                    Showing <strong>{filteredCertificates.length}</strong> of <strong>{certificates.length}</strong> results
                </div>
                <div className="flex gap-2">
