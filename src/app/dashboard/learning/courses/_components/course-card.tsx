        ...
        <Card className="group overflow-hidden card-hover border-border/50 bg-card/50 backdrop-blur-sm flex flex-col h-full">
            <div className="relative">
                <AspectRatio ratio={16 / 9} className="bg-muted">
                    ...
                    <div className="absolute top-3 left-3 z-20">
                        <Badge variant="secondary" className="bg-background/80 backdrop-blur text-xs font-semibold">
                            {course.category}
                        </Badge>
                    </div>
                    <div className="absolute top-3 right-3 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button size="icon" variant="secondary" className="h-8 w-8 rounded-full bg-background/80 backdrop-blur">
                    ...
                    <div className="absolute bottom-3 left-3 right-3 z-20 flex justify-between items-end">
                        {course.progress > 0 && (
                            <div className="w-full">
                                ...
                                <Progress value={course.progress} className="h-1.5 bg-background/30 [&>div]:bg-primary" />
                            </div>
                        )}
                    </div>
                </AspectRatio>
            ...
            <CardHeader className="p-5 pb-2">
                <div className="flex justify-between items-start mb-2">
                    <Badge variant={
                        course.level === "Advanced" ? "destructive" :
                            course.level === "Intermediate" ? "default" : "secondary"
            ...
            <CardFooter className="p-5 pt-0 mt-auto flex items-center justify-between text-muted-foreground text-xs border-t bg-muted/20">
                ...
                <Button size="sm" variant={course.progress > 0 ? "default" : "outline"} className="h-8 ml-auto">
