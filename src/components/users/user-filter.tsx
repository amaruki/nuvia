...
import { UserFilter as UserFilterType, UserStatus, AuthStatus } from "@/types/user-management.types"
import { type UserRole, ROLE_DISPLAY_INFO, isPredefinedRole } from "@/types/dashboard.types"
import { ChevronDown, ChevronUp, Search, X, Calendar, MapPin, Shield, Mail, Phone } from "lucide-react"
...
export function UserFilter({ filters, onFilterChange, onClearFilters, className }: UserFilterProps) {
  ...
  const handleAuthStatusChange = (authStatus: AuthStatus, checked: boolean) => {
    ...
    onFilterChange({ ...filters, authStatuses: newAuthStatuses.length ? newAuthStatuses : undefined })
  }
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <FilterSection title="User Roles" icon={<Shield className="size-4" />}>
          <div className="space-y-4">
            ...
            {[
              { category: 'administrative', title: 'Administrative', roles: ['superadmin', 'admin'] },
              { category: 'leadership', title: 'Leadership', roles: ['treasurer', 'chapter_president', 'chapter_admin', 'committee_chair'] },
              { category: 'staff', title: 'Staff', roles: ['staff', 'organizer', 'moderator'] },
              { category: 'membership', title: 'Membership', roles: ['member_corporate', 'member_professional', 'member_student', 'member', 'user'] }
                    ...
                    return (
                      <div key={role} className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => handleRoleChange(role as any, checked as boolean)}
                        />
                        <Label htmlFor={`role-${role}`} className="text-sm font-normal cursor-pointer flex-1">
                          <div className="flex items-center gap-2">
                            {roleInfo ? (
                              <>
            ...
            {Object.values(AuthStatus).map((authStatus) => (
              <div key={authStatus} className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => handleAuthStatusChange(authStatus, checked as boolean)}
                />
                <Label htmlFor={`auth-${authStatus}`} className="text-sm font-normal">
                  {authStatus.replace("_", " ").charAt(0).toUpperCase() + authStatus.replace("_", " ").slice(1)}
                </Label>
              </div>
            ))}
          </div>
