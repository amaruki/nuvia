...
export function useArticles(): UseArticlesReturn {
  ...
  const filteredArticles = useMemo(() => {
    ...
    if (filters.series && filters.series.length > 0) {
      filtered = filtered.filter(article => 
        article.series && filters.series!.includes(article.series.id)
      );
    }
    ...
    if (filters.minEngagement !== undefined) {
      filtered = filtered.filter(article => article.metrics.engagementScore >= filters.minEngagement!);
    }
    if (filters.maxEngagement !== undefined) {
      filtered = filtered.filter(article => article.metrics.engagementScore <= filters.maxEngagement!);
    }
  ...
  // Get single article
  const getArticle = useCallback((id: string): Article | null => {
      ...
      setArticles(prev => prev.map(article => {
        if (article.id === id) {
          return {
            ...
            scheduledFor: data.status === 'scheduled' ? data.scheduledFor : article.scheduledFor
          } as Article;
        }
        return article;
      }));
  ...
  const duplicateArticle = useCallback(async (id: string): Promise<Article> => {
    try {
  ...
  // Status management
  const publishArticle = useCallback(async (id: string): Promise<void> => {
    await updateArticle(id, { status: 'published', publishedAt: new Date() });
  }, [updateArticle]);

  const archiveArticle = useCallback(async (id: string): Promise<void> => {
    await updateArticle(id, { status: 'archived' });
  }, [updateArticle]);

  const scheduleArticle = useCallback(async (id: string, date: Date): Promise<void> => {
    await updateArticle(id, { status: 'scheduled', scheduledFor: date });
  }, [updateArticle]);

  const unpublishArticle = useCallback(async (id: string): Promise<void> => {
    await updateArticle(id, { status: 'draft' });
  }, [updateArticle]);

  const reviewArticle = useCallback(async (id: string, reviewerId: string): Promise<void> => {
  ...
  // Utility functions
  const exportArticles = useCallback((format: 'csv' | 'json' | 'pdf') => {
  ...
  const importArticles = useCallback(async (file: File): Promise<void> => {
    try {
