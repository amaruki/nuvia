...
interface UseInvoiceMutationsDeps {
  /** Invalidates the shared finance cache after a successful action. */
  invalidate: () => Promise<void>;
}

/**
 * C3-backed invoice actions: record a payment (POST /finance/payments) and
 ...
 */
export function useInvoiceMutations({ invalidate }: UseInvoiceMutationsDeps) {
  ...
  const voidInvoiceMutation = useMutation({
    mutationFn: async (invoiceId: string) => {
      const { data } = await apiFetch<{ invoice: unknown }>(
        ...
        {
          method: "POST",
          body: JSON.stringify({ reason: "Voided from the invoices dashboard" }),
        },
      );
      return data;
    },
