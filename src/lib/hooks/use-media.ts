...
interface UseMediaReturn {
  ...
  grantPermission: (mediaId: string, permission: Omit<MediaPermission, 'id' | 'grantedAt'>) => Promise<void>;
  revokePermission: (permissionId: string) => Promise<void>;
  
  // Analytics
  getAnalytics: (mediaId: string, dateRange?: { start: Date; end: Date }) => Promise<MediaAnalytics[]>;
...
export const useMedia = (): UseMediaReturn => {
  ...
  // CRUD operations
  const uploadMedia = useCallback(async (files: File[], options?: MediaUploadOptions) => {
    setLoading(true);
    setError(null);
    
    try {
      const uploadPromises = files.map(file => MediaService.createMedia({
        title: file.name,
  ...
  const duplicateMedia = useCallback(async (id: string) => {
    try {
  ...
  // Folder management
  const createFolder = useCallback(async (data: Omit<MediaFolder, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
  ...
  const updateFolder = useCallback(async (id: string, data: Partial<MediaFolder>) => {
    try {
      ...
      setFolders(prev => prev.map(item =>
        item.id === id ? { ...item, ...updatedFolder } : item
      ));
    } catch (err) {
  ...
  // Permission management
  const grantPermission = useCallback(async (mediaId: string, permission: Omit<MediaPermission, 'id' | 'grantedAt'>) => {
    try {
      // This method doesn't exist in MediaService, so we'll implement a basic version
  ...
  // Analytics
  const getAnalytics = useCallback(async (mediaId: string, dateRange?: { start: Date; end: Date }) => {
    try {
      ...
      setMedia(prev => prev.map(item =>
        item.id === mediaId ? { ...item, analytics: [analytics] } : item
      ));
  ...
  const bulkUpdate = useCallback(async (ids: string[], data: Partial<MediaFormData>) => {
    try {
  ...
  const bulkMove = useCallback(async (ids: string[], folderId: string) => {
    try {
      ...
      // Update in state
      setMedia(prev => prev.map(item =>
        ids.includes(item.id) ? { ...item, folderId } : item
      ));
    } catch (err) {
  ...
  const importMedia = useCallback(async (file: File) => {
    setLoading(true);
    setError(null);
