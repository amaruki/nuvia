  ...
  const generateMockMembers = useCallback(
    (
      ...
      return new Promise((resolve) => {
        setTimeout(
          () => {
            const mockMembers: MembershipProfile[] = Array.from(
              { length: pageSize },
              (_, index) => {
                ...
                const lastName =
                  lastNames[Math.floor(globalIndex / firstNames.length) % lastNames.length];
                ...
                return {
                  ...
                  membershipStatus: statuses[
                    Math.floor(globalIndex / 6) % statuses.length
                  ] as MembershipStatus,
                  ...
                  skills: ["JavaScript", "React", "Node.js", "TypeScript", "Leadership"].slice(
                    0,
                    Math.floor(Math.random() * 4) + 2,
                  ),
                  interests: ["Technology", "Innovation", "Networking", "Mentoring"].slice(
                    0,
                    Math.floor(Math.random() * 3) + 1,
                  ),
                  committees: ["Events", "Membership", "Professional Development"].slice(
                    0,
