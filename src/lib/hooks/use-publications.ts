...
export function usePublications(): UsePublicationsReturn {
  ...
  const filteredPublications = useMemo(() => {
    ...
    if (filters.tags && filters.tags.length > 0) {
      filtered = filtered.filter(pub =>
        pub.tags.some(tag => filters.tags!.includes(tag.id))
      );
    }
  ...
  const getPublication = useCallback((id: string): Publication | null => {
  ...
  const addPublication = useCallback(async (data: PublicationFormData): Promise<Publication> => {
    try {
      ...
      const newPublication: Publication = {
        ...
        status: data.status,
        author: mockPublications.find(p => p.author.id === data.authorId)?.author || mockPublications[0].author,
  ...
  const updatePublication = useCallback(async (id: string, data: Partial<PublicationFormData>): Promise<Publication> => {
    try {
      // Simulate API call
      await new Promise(resolve => setTimeout(resolve, 1000));
      ...
      setPublications(prev => prev.map(pub => {
        if (pub.id === id) {
          return {
            ...
            ...data,
            lastModified: new Date(),
            publishedAt: data.status === 'published' && pub.status !== 'published' ? new Date() : pub.publishedAt,
  ...
  const duplicatePublication = useCallback(async (id: string): Promise<Publication> => {
    try {
  ...
  // Status management
  const publishPublication = useCallback(async (id: string): Promise<void> => {
    await updatePublication(id, { status: 'published', publishedAt: new Date() });
  }, [updatePublication]);

  const archivePublication = useCallback(async (id: string): Promise<void> => {
    await updatePublication(id, { status: 'archived' });
  }, [updatePublication]);

  const schedulePublication = useCallback(async (id: string, date: Date): Promise<void> => {
    await updatePublication(id, { status: 'scheduled', scheduledFor: date });
  }, [updatePublication]);

  const unpublishPublication = useCallback(async (id: string): Promise<void> => {
  ...
  // Utility functions
  const exportPublications = useCallback((format: 'csv' | 'json' | 'pdf') => {
  ...
  const importPublications = useCallback(async (file: File): Promise<void> => {
    try {
