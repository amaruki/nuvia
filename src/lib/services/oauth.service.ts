...
export class OAuthService {
  ...
  static async isEmailRegisteredWithOAuth(email: string): Promise<boolean> {
    try {
      ...
      throw new BusinessLogicError(
        'Failed to check OAuth registration',
        'OAUTH_CHECK_FAILED'
      );
    }
  }
   ...
   */
  static async validateOAuthSignIn(email: string, provider: string): Promise<{ isValid: boolean; error?: string }> {
    try {
