...
/**
 * Helper function to get an appropriate icon based on login activity status and device type
 * @param successful - Whether the login was successful
 * @param deviceType - The type of device used for login
 * @returns React component with the appropriate icon
 */
export function getActivityIcon(successful: boolean, deviceType: string) {
  if (!successful) {
    return (
...
/**
 * Helper function to get an appropriate icon for device type
...
export function getDeviceIcon(deviceType: string) {
  switch (deviceType.toLowerCase()) {
    ...
    case 'phone':
      return (
        <svg className="h-5 w-5 text-foreground/40" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" />
        </svg>
      );
    case 'tablet':
